...
#| Usage: waitCellOn.py [-n GSM|WCDMA|LTE] [-t search_duration (s)] [-o <equipment_name>]           |
#|   example: python waitCellOn.py -n LTE -o TCPIP0::CMW50050-144593::inst0::INSTR                  |
#+--------------------------------------------------------------------------------------------------+
...
def main(argv):
    ...
    try:
        ...
        print (str(e))
        print("Usage: waitCellOn.py [-n GSM|WCDMA|LTE] [-t search_duration (s)] [-o <equipment_name>]")
        sys.exit(2)
    ...
    try:
            ...
            if network != "ALL":
                ...
                for nw in networkCommand:
                    ...
                    if ("OFF" not in queryReturn):
                        ...
                        break
            if ("OFF" not in queryReturn):
                print("\t<- Network %s online" % network)
                cmw.close()
                break
        ...
        cmw.close()
        sys.exit(2)
    if ("OFF" in queryReturn):
        print("\t<- ERROR: The network %s is not ON" % network)
    print "------------------------------------------------------\n"
